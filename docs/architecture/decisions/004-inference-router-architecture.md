...
- **`bytes_per_kv_token`, `attention_arch`, `slot_class` fields: not yet added.** Pure-data extension still pending — now sequenced as the next chunk after this status entry, since the runtime probe is in place to validate the numbers in situ rather than guessing in isolation.
