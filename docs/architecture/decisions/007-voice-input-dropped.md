...
**Status:** Accepted
**Date:** 2026-04-27
**Related:** [`docs/features/voice.md`](../../features/voice.md)

## Context

The product was originally pitched as voice-first — voice input + voice output as the primary interface to a local knowledge system. The product-direction research records that voice-first positioning was an unvalidated product-differentiation bet, never tested with real users. The hybrid-deployment math, the licensing math, and the compliance math all work with or without voice.
...
- An always-on listening surface that is its own privacy / compliance liability — a microphone hot in a privileged-work environment is exactly the kind of risk a legal operator's IT review will flag.
...
The model-research recorded the decision to drop voice input from v1, preserving TTS output (Kokoro-82M, ~80 MB, Apache 2.0). The rationale: TTS is cheap, has zero privacy / always-on liability, and preserves a meaningful slice of the "Jarvis talks to you" experience without input-side complexity.
...
1. **The voice-first claim is unvalidated.** No deep-dive tested whether voice input matters to engineering or legal operators. Marketing it without evidence is a credibility risk.
...
- **Marketing positioning** shifts from "voice-first" to **"local-first knowledge system that talks back"** (or equivalent). Update at minimum: [`overview.md`](../../overview.md), the workspace-onboarding feature doc, and any onboarding UI strings.
...
### Negative
- The "voice-first" pitch in earlier planning and marketing material is retired. Some prior-thinking artifacts are now superseded.
...
1. **Marketing copy sweep.** Voice-first phrasing across [`overview.md`](../../overview.md), onboarding strings, README, and any pitch-deck artifacts. Replace with the new positioning.
