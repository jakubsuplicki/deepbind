...
**Status:** Accepted
**Date:** 2026-04-30
**Related:** [ADR 002](002-pure-local-product-shape.md) · [ADR 003](003-desktop-distribution-tauri-and-sidecars.md) · [ADR 012](012-chat-model-self-test.md) · [`docs/research/models/model-research-4.md`](../../research/models/model-research-4.md)
...
[`docs/research/models/model-research-4.md`](../../research/models/model-research-4.md) (final, 2026-04) does the research work — license filter, qualifying universe, hardware tiering, recommended primary stack — but was never crystallized as a load-bearing decision document. This ADR locks it in.

## Decision drivers

1. **License purity** — Apache 2.0 / plain MIT only. No Gemma Terms of Use flow-down, no DeepSeek custom license, no Z.ai (Entity-Listed since Jan 2025), no Llama community license. Compliance buyers in defense / privacy / regulated verticals do an *outbound license review* of every model in the bundle; one non-permissive entry blocks procurement. Per research-4 §"License filter (settled)."
...
3. **Hardware heterogeneity is real** — the user we sell to runs anything from a 16 GB compliance laptop to a dual-GPU workstation. A one-size-fits-all default either underutilizes the workstation or OOMs the laptop. Tiering is the architectural answer.
4. **First-run UX is the first compliance impression** — the buyer's first session decides whether they trust the product. Crashing during onboarding because we picked too-big a default is irrecoverable. **Conservative-default + opt-in upgrade** beats **aspirational-default + crash-loop**.
...
The full per-tier catalog is the **Recommended primary stack (final)** section of [research-4.md](../../research/models/model-research-4.md#recommended-primary-stack-final). This ADR adopts that catalog verbatim as the user-pickable surface; entries that don't appear there are not user-pickable in v1.
...
| Compliance reasoning (opt-in) | OLMo 3.1-32B-Think | Q4_K_M | ~18 GB | for compliance buyers who want the most-permissive-license stack |
...
2. **Pre-flight memory check** — before each turn, [`effective_footprint_bytes()`](../../../backend/services/ollama_service.py) (already exists for ADR 012) plus current free RAM (via `psutil`) is compared against an 80% headroom threshold. Below threshold → swap *before* the request. Avoids the user-visible OOM-then-retry round trip.
...
The bundled set is intentionally minimal. Every additional bundled byte is a license review the buyer does before procurement; every additional bundled model is a notarization re-cert when it's bumped. v1 ships only the always-resident plumbing.

## Alternatives considered

- **Static default model regardless of hardware** (e.g. always pull Qwen3-4B). Avoids the probe complexity. Rejected: a buyer on a 64 GB workstation would correctly conclude we shipped them a toy. Conversely, a static Qwen3-30B default on a 24 GB Mac OOMs in onboarding. The asymmetry argues for tiering even at the cost of probe code.
- **No first-run pull — start with no chat model, surface a picker.** Cleanest from a "what does the bundle do?" perspective. Rejected: the first-run experience becomes "click around for ten minutes choosing a model"; chat doesn't work for the duration. Onboarding momentum dies. Buyers in compliance verticals are not patient with empty-state UX.
- **Bundle one chat model in the installer.** Eliminates first-run pull. Rejected for v1: the installer surface is part of what the buyer license-reviews; bundling 5–80 GB of chat-model weights *into the signed installer* triggers a much bigger compliance review than fetching the same blob via signed manifest at first run. Also: the right model for one buyer's hardware is the wrong model for another's. Tiered first-run pull side-steps both.
...
- Hardware-aware first-run UX gives a sane default for every buyer's machine.
- Downgrade ladder makes memory pressure a graceful degradation, not a failure mode. Compliance buyers explicitly ask "what happens when the laptop is constrained?" — this is the answer.
...
5. **Per-vertical model packs** — research-4 §"Strategic angle" hints at a defense-engineering pack, a legal pack, etc. Out of scope for v1; revisit when the engineering vertical starts pre-selling.
