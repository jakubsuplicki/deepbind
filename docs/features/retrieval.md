...
| `JARVIS_RERANKER_MODEL` | `onnx-community/bge-reranker-v2-m3-ONNX` | Override the cross-encoder model. Bundled inside the `.app` (~570 MB INT8) per ADR 003 §A; loads on first reranker call. |
...
- **Cosine is disabled entirely under `JARVIS_DISABLE_EMBEDDINGS=1`.** The test suite sets this env var by default (see `conftest.py`) to avoid loading the ~1 GB fastembed model on every run. Tests that need the full pipeline must clear the var and stub the embedding functions to stay fast.
