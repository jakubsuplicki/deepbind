...
**Re-warming microstate (warm-sidecar > 1 s wait).** Between the cold-bootstrap warmup and the silent typing-dots indicator there's a third class of wait: warm-sidecar follow-up turns where the model has produced output before but the next turn still hits more than a second of pre-stream latency. The chat-step instrumentation at [`backend/routers/chat.py:861`](../../backend/routers/chat.py) (`before_stream_response` step log) was added precisely to measure the consistently-reproducing turn-2-after-fresh-launch back-pressure (~24 s observed) where the dots used to sit silently. [`ChatPanel.vue`](../../frontend/app/components/ChatPanel.vue) now starts a 250 ms timer when `isLoading` flips true and, after 1 s without any text or tool activity, swaps the dots for a one-line italic "re-warming model…" hint with a single pulsing neon-cyan dot. The threshold and copy are deliberately understated — this is not the 4-stage instrument readout, it's a small "I'm still here, the model is loading" reassurance for the silent-dots case. Cleared when streaming starts (the typing dots and re-warming hint both gate on `!currentResponse`); timer is cleared on `isLoading` falling and on component unmount. Cold-sidecar first turn keeps using `ChatFirstTurnWarmup` — re-warming covers everything that falls through to the typing-dots path.
