...
- **[`chat_adapters.py`](../../backend/tests/eval/conversations/chat_adapters.py)** — the concrete `ChatCallable` implementation. **`OllamaChat` is the only adapter** (talks to `http://127.0.0.1:11434`, the v1 canonical chat model, temperature 0, fixed seed) so the eval measures the production stack honestly. This is a local-only build (ADR 015 removed all cloud LLM SDKs); there is no hosted-API adapter or escape hatch. The `make_chat(provider="ollama")` factory returns it (any other provider name raises); `make_chat_factory(provider=...)` returns a `seed → ChatCallable` factory used by the multi-seed runner. Canonical content blocks (text / tool_use / tool_result) are translated to Ollama's native message shape at the adapter boundary, including the tool_use → tool_calls and tool_result → tool-role-message conversions. **Chain-of-thought stripping** (`_strip_thinking`) runs on every Ollama response: Ollama 0.18 with `think: false` strips the opening `<think>` tag but leaves the model's reasoning prose plus `</think>` in the body, so the adapter discards everything up to and including the last `</think>` before returning. Without this, `must_not_contain` guards regex over the chain-of-thought and trip on rejected-candidate names that the model only mentioned in reasoning — false-positive confabulation rates as high as 30+ pp on guard-heavy fixtures.
...
2. **Active-model lookup.** `_resolve_system_prompt_budget` and `_maybe_compact` both look up the active local model in `MODEL_CATALOG` via `get_model_by_ollama_model` — the lookup key is the raw Ollama tag (e.g. `qwen3:8b`). The legacy `ollama_chat/` LiteLLM-style prefix is gone from the wire protocol end-to-end (2026-05-04 cleanup); a defensive prefix-strip remains in `OllamaDispatcher` and a few display helpers for back-compat with older session-history records.
...
A sibling harness at [`backend/tests/eval/latency/`](../../backend/tests/eval/latency/) measures user-perceived chat latency (TTFT, decode tokens/sec, end-to-end wall clock) on the actual ICP hardware. Same discipline as the conversation harness — committed JSON baselines, opt-in pre-merge gate, bootstrap-CI verdict — applied to the speed axis instead of answer-quality. Runs the streaming Ollama HTTP `/api/chat` endpoint with `temperature: 0` and a fixed seed, captures TTFT from the first non-empty content delta, and reads `eval_count / eval_duration` from the `done` event for honest decode throughput. Local-only build (ADR 015): every scenario runs against the local Ollama stack — there is no hosted-API reference scenario. CLI: `python -m tests.eval.latency.run_bench`. See [`docs/concepts/latency-baseline.md`](../concepts/latency-baseline.md) and [ADR 011](../architecture/decisions/011-latency-benchmark-harness.md).
...
**Sidecar ML warmup ([`backend/services/warmup_service.py`](../../backend/services/warmup_service.py)).** *Started 2 s after lifespan yield — see "GIL deferral" below for why.* The chat hot path lazy-loads four heavy artefacts on first use: the fastembed embedder (~3 s, ~1 GB RAM), the fastembed cross-encoder reranker (~5-8 s, ~600 MB RAM), the spaCy NER pipeline `xx_ent_wiki_sm` (~2 s), and the HuggingFace `tokenizers` Rust extension plus per-model `tokenizer.json` files (~50-200 ms each). Without warmup those costs land on user-facing turns in a brutal pattern: turn 1 of a cold session has empty history so the retrieval pipeline short-circuits and only spaCy fires (during `_save_session_bg`); turn 2 then runs the full retrieval pipeline (lazy-loads embedder + reranker) AND contends with the still-loading spaCy import — producing a 25-30 s wall-clock stall that the perf instrumentation surfaces as `rust_to_fastapi_ms ≈ 20000+` (the FastAPI `accept()` is starved by GIL-held Python work on the asyncio thread). The fix moves all of those costs to sidecar boot. A daemon `threading.Thread` started from the FastAPI lifespan (after `start_workers()`, non-blocking on the `yield`) sequentially exercises each component on dummy input: tokenizer (walks `_BUNDLED_TOKENIZER_IDS` so every catalog model has its tokenizer hot), spaCy NER (`extract_entities` over a synthetic sentence), embedder (`embed_query("warmup")` — exercises both ONNX load and the query-prefix path), reranker (`rerank("warmup query", [doc1, doc2])`). Sequential ordering — tokenizer → NER → embedder → reranker — chosen so a chat dispatch racing the warmup hits the smallest-and-most-likely-needed components first; parallel ONNX loads contend for the same CPU + memory bandwidth so they aren't actually faster. Per-component states progress `pending` → `running` → `ready` / `failed` / `skipped` (`skipped` is not an error: `JARVIS_DISABLE_RERANKER=1` or a stripped `tokenizers` package returns `skipped`, which the chat path's documented fallbacks already handle). The orchestrator wraps each warmer in its own try/except so a rogue future warmer can't abort the loop. `start()` is idempotent — calling it twice is a no-op. Status is exposed via `GET /api/health/warm` (always 200 — readiness is a payload field, not an HTTP status) returning `{ready, started, completed, started_at, completed_at, components: {name: {state, duration_ms, error}}}`.

**GIL deferral for warmup start (ADR 022 follow-up).** spaCy's `xx_ent_wiki_sm` import + first inference holds Python's GIL for ~30-40 s, which starves the asyncio event loop and queues every inbound HTTP request behind the load. Live measurement on M5 24 GB after the splash landed: the Tauri shell's `POST /api/license/state` probe (which gates the splash → real-layout transition) sat in the accept queue for **40 s** because the warmup thread had grabbed the GIL during NER load. The fix: `main.py::lifespan` schedules `warmup_service.start()` via `asyncio.create_task` with a 2 s `asyncio.sleep(2)` head-start, so the first inbound HTTP requests after `JARVIS_BACKEND_READY` (license probe, health check, warmup-status poll) get served on the uncontended event loop, then warmup runs in the background while the user is already in the real UI. Net result: splash transition fires within ~1 s of sidecar READY (vs. 40 s prior); warmup still completes ~42 s later, but in the background where it doesn't block anything user-visible.

**Frontend warmup surface.** [`useWarmup.ts`](../../frontend/app/composables/useWarmup.ts) is a module-level singleton — only ever one poll loop across the app, even with multiple consumers — that polls `/api/health/warm` with a backoff schedule (`250, 250, 500, 500, 1000, 1000, 2000` ms then 2 s steady-state) and stops the instant `ready: true` comes back. The poll is started once at the [`default.vue`](../../frontend/app/layouts/default.vue) layout level so it begins as early as possible (during workspace init, before the user has even reached the chat page). [`ChatPanel.vue`](../../frontend/app/components/ChatPanel.vue) renders a small instrument-readout pill — `PREPARING MODELS · tokenizer · NER · embedder · reranker` — above the messages area when `started && !ready`; per-component states are styled (running = phosphor amber, ready/skipped = struck-through with amber underline, failed = neon orange), and the pill transitions out the instant warmup completes via `<Transition name="warmup-pill">`. The pill never blocks input — if the user dispatches a chat turn while warmup is still running, the chat path simply takes the lazy-load hit on whatever isn't yet loaded, which now degrades to "the warmup we already showed you, but on the chat path" rather than a silent 25-30 s stall. In practice warmup completes in ~10-15 s on M5 24 GB after a warm cache; the user typing their first message takes longer than that on average, so the chat path almost always finds the components already hot.

**Post-turn save runs on a thread pool, not the event loop.** When a chat turn completes, the chat router fires `_schedule_session_save(session_id)` to persist the conversation to `memory/conversations/<note>.md`. The save flow ([session_service.py](../../backend/services/session_service.py) `save_session_to_memory` → `_save_session_to_memory_locked`) runs inside an `asyncio.ensure_future` task — but two of its inner steps are **CPU-bound, GIL-held Python work** that must not run on the event loop: `_extract_people_from_messages` (calls `services.entity_extraction.extract_entities`, which lazy-imports spaCy and runs NER inference) and `graph_service.ingest_note` (which routes through `entity_edges.extract_entities` for the same NER pass). On a cold app start, the **first** call to either takes 10-20 seconds because spaCy lazy-loads its language model on import + first inference. If those calls run inline on the asyncio thread, the GIL is held for the full duration and uvicorn's `accept()` syscall is starved — so the user's next-turn TCP SYN sits in the kernel accept queue for ~20 s and surfaces as `chat_step received rust_to_fastapi_ms=18000-23000` (with `fastapi_to_lock_ms ≈ 0.1 ms`, confirming the route handler runs immediately once it finally gets accepted). The fix is `await asyncio.to_thread(...)` on both call sites — spaCy runs on a worker thread, the event loop stays responsive throughout, and turn 2 lands as fast as any subsequent turn. The pattern only manifested on cold-launch turn 2 because spaCy stays cached after first load, so subsequent saves are millisecond-fast and don't block.

**Per-WS outbound send queue.** [`backend/services/ws_send_queue.py`](../../backend/services/ws_send_queue.py) decouples backend chat dispatch from WebSocket write back-pressure. Without it, `_send_event(ws, ...)` calls `await ws.send_json(...)` synchronously inside the per-session dispatch lock — and on macOS WKWebView, App Nap can pause the JS-side WS read pump after the user's UI goes idle (between turns: the final `done` event fires, `isLoading=false`, page goes quiescent, WKWebView naps the JSContext). The kernel TCP receive buffer fills, the server's `await ws.send_json(...)` for the *previous* turn's trailing `done` event blocks, and the lock stays held until the user's next keystroke wakes WKWebView 20-30 s later. ADR 016 attempted to fix this by routing the user's *outbound send* off the WebSocket onto HTTP+IPC; that helped a different failure mode but didn't address the lock-wait because the bottleneck is on the *receive* direction back into the WebView. The send queue fixes it structurally: each WS gets a bounded `asyncio.Queue` plus a single consumer task that drains it in FIFO order. `_send_event` becomes a fast `await queue.put(...)` that returns once the frame is enqueued, not when it's been flushed to the kernel; the dispatch lock is released as soon as the model work completes. Frame ordering is preserved by single-consumer-per-WS (turn 1's `text_delta`s arrive before turn 1's `done` arrives before turn 2's `text_delta`s). The queue is bounded (default 256 frames) so a permanently-stuck consumer can't leak memory; `enqueue()` blocks with a 2 s timeout when full. Send failures fail-latch — once `ws.send_json` raises, the consumer exits and subsequent enqueues fast-return False, leaving the WS receive loop to see `WebSocketDisconnect` and tear down on its own. Test surface: 12 unit tests in `test_ws_send_queue.py` cover FIFO + back-pressure-decoupling + fail-latching + close semantics; 4 integration tests in `test_chat_ws.py` pin the chat-router contract (queue is attached on connect, detached on disconnect, `_send_event` routes through it, and a wedged `ws.send_json` does not couple subsequent `_send_event` calls).

**Slow-warm-turn microstate (warm-sidecar > 3 s wait).** Between the cold-bootstrap warmup and the silent typing-dots indicator there's a third class of wait: warm-sidecar follow-up turns where prefill compute takes noticeably longer than the post-prefix-stable-fix baseline (1–2 s on M5 24 GB / Qwen3 8B / 2.7K-token prompt). [`ChatPanel.vue`](../../frontend/app/components/ChatPanel.vue) starts a 250 ms timer when `isLoading` flips true and, after **3 s** without any text or tool activity, swaps the dots for a one-line italic **"thinking…"** hint with a single pulsing neon-cyan dot. The microstate originally fired at >1 s with copy "re-warming model…" — that was the right shape for the 7–24 s prefix-instability stalls *before* the ADR 009 amendment 2026-05-01 fix landed (system-prompt prefix-stable + retrieval attached to user message). With the fix in place warm turns sit at 1–2 s prefill compute, so a 1 s threshold tripped on every normal turn and the "re-warming" copy was misleading because the model isn't actually being re-warmed (`load_ms` ≈ 50 ms means the model is resident — this is just prefill compute over the user's new tokens + retrieval block). The 3 s threshold + "thinking…" copy means this surface only appears when prefill genuinely exceeds the post-fix range (e.g. a large retrieval block, a tool-loop follow-up dispatch, or a slower hardware path). Cleared when streaming starts (the typing dots and slow-turn hint both gate on `!currentResponse`); timer is cleared on `isLoading` falling and on component unmount. Cold-sidecar first turn keeps using `ChatFirstTurnWarmup`.
