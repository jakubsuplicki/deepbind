...
For business document files, `source_import.extractors` converts the approved upload into Markdown before indexing. DOCX/XLSX/PPTX use conservative standard-library ZIP/XML parsing so the desktop bundle does not gain new parser dependencies in this slice. Office core-property titles are preserved when present, with filenames as the fallback. Malformed Office XML and encrypted Office/ZIP members fail with safe importer errors instead of raw parser exceptions. HTML uses the existing `trafilatura` + `markdownify` stack and strips script/style/template content on fallback conversion; if main-content extraction fails, the visible warning says the page body was converted rather than exposing parser exception names. RTF uses a best-effort control-word stripper, EML uses Python's standard `email` package with `Cc` headers and attachment names as metadata only, and single-file ZIP upload creates a safe archive inventory note rather than extracting child files. When a best-effort extractor has warnings, `fast_ingest` writes them into `extractor_warnings` frontmatter, appends an `Import warnings` section to the generated Markdown, and returns the warnings to source import so completion can show imported-with-warnings counts and per-file warning details. Current warning examples include workbook row/column preview caps, malformed optional shared-string tables, blank decks, unreadable RTF/email bodies, listed-but-not-imported email attachments, archive inventory preview limits, HTML fallback, and generated Markdown truncation.
...
`ImportDialog` uses a drag-and-drop zone backed by a hidden `<input type="file">` and submits file imports via `multipart/form-data` using Nuxt's `$fetch`. It also has an in-progress Folder mode for [Folder Source Import](folder-source-import.md): the desktop shell grants a selected folder to `/api/source-import/scan`, the dialog shows a metadata-only inventory plus review exclusions, scan issues can be copied as a relative-path report, the approved selection can start a local import batch with progress/completion counts, active batches can be cancelled, terminal imports can be removed or scanned again for new/changed files from the completion surface or recent import history, and completed imports can jump back to the generated notes in Memory. `LinkIngestDialog` uses `v-model` for open/close state and performs client-side URL type detection with the same regex patterns used on the backend, giving the user immediate visual feedback before submitting.
...
- `backend/services/source_import/extractors.py` — Best-effort business document extractors for DOCX, XLSX, PPTX, HTML/HTM, RTF, EML, and safe single-file ZIP inventory notes; direct coverage includes malformed XML and encrypted archive guards in `backend/tests/test_source_import_extractors.py`
- `backend/services/source_import/archives.py` — ZIP directory scanning, archive safety guards including encrypted-member rejection, and temporary extraction for approved folder-import or standalone archive children
- `backend/services/source_import/limits.py` — Shared scan, preview, per-file, approved-batch, archive, and source-import concurrency caps used before approved files become Markdown memory
...
- `backend/services/source_import/manifest.py` — SQLite operational manifest for approved folder imports, including duplicate policy, batch/file status, extractor warnings, and generated note paths
...
- `backend/tests/test_source_import_scan.py` — Folder import lifecycle coverage, including bundled sample dataset end-to-end import, safe provenance, completion summaries, and import-scoped retrieval traces
...
- `frontend/app/composables/useSourceImport.ts` — Frontend API wrapper for trusted folder selection, scan, review selection, import start with duplicate policy, import history listing, import status polling, cancellation, rescan, and import removal
- `frontend/app/composables/useFolderSourceImportDialog.ts` — Folder import dialog state machine for scan review, exclusion rules, recent import history, progress polling, duplicate-policy start, cancellation, rescan, removal, completion summaries, and skipped-file hints
- `frontend/app/components/ImportDialog.vue` — Drag-and-drop file upload modal; submits to `/api/memory/ingest` as multipart and renders the folder source import shell and recent history surfaces through `useFolderSourceImportDialog`
- `frontend/app/components/FolderSourceImportDetail.vue` — Shared folder import batch detail surface for active imports and reopened history items, including progress, completion, imported-with-warnings counts, open-imported-notes, skipped-file review, scan-again, cancel, and remove controls
