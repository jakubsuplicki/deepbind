...
- **No token-budget UI.** The settings page does not expose a daily token budget. The `check_budget()` function exists in the codebase but is not currently consumed; if reintroduced, the gate must apply at a controllable surface (chat router) and ship with a UI to raise the limit.
